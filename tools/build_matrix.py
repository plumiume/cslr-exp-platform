...
使用例:
    ...
    # YAMLファイルを明示指定
    uv run python tools/build_matrix.py my-matrix.yaml --all
    ...
    # ビルド後にレジストリへプッシュ（従来の repo 例）
    uv run python tools/build_matrix.py --all --push localhost:5001
"""
...
def main(
    ...
    push_registry: str | None = typer.Option(
        None,
        "--push",
        metavar="[repo]",
        help="ビルド後に指定 repo へプッシュ (例: --push localhost:5001)",
    ),
    ...
    progress: str = typer.Option(
        "auto",
        "--progress",
        help="Docker ビルドの進捗表示モード (auto / plain / tty)",
    ),
    matrix_file: Path = typer.Argument(
        Path("build-matrix.yaml"),
        help="ビルドマトリックス設定ファイル (YAML)",
    ),
) -> None:
    """Docker Build Matrix Runner"""
    # 設定ファイルを読み込み
    ...
    for idx, (config, target_info) in enumerate(builds, 1):
        ...
        if success and not dry_run:
            ...
            # レジストリにプッシュ
            if push_registry:
                push_to_registry(full_tag, registry_url=push_registry)
