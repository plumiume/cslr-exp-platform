...
- `cluster_test.target` は `services.ray.target` から独立して指定できます。
- テスト系コンテナとメイン Ray ノードは `services.ray.default` と `services.ray.cpu/gpu` の上書き合成で解決した値を使います。
- `ray-cpu` は常に生成され、`ray-gpu` は `host.has_gpu=true` のとき生成されます。
...
graph TB
    subgraph "Host Machine"
        subgraph "ray-network (172.28.0.0/16)"
            ...
            ray_cpu["ray-cpu<br/>resolved ports from config"]
            ray_gpu["ray-gpu<br/>resolved ports from config"]
        ...
        test_client -->|Ray Client API<br/>ray://HOST_IP:target_client_port| ray_cpu
...
- **Image**: `services.ray.cpu.image ?? services.ray.default.image`
- **Hostname**: `ray-cpu`
- **Whitelist**: ✅ (`config.yaml` の `nodes.head_whitelist` に含まれる)
- **起動モード**: HEAD
  - Health service 確認 → ✓
  - 既存 HEAD 検索 → なし
  - `ray start --head --dashboard-host=0.0.0.0 --port=<resolved head_port>`
- **Ports**: `head_port`, `dashboard_port`, `client_port`（いずれも resolved 値）

#### 3. ray-gpu (WORKER)
- **役割**: メインクラスタの WORKER ノード
- **Image**: `services.ray.gpu.image ?? services.ray.default.image`
- **Hostname**: `ray-gpu`
...
- **Image**: `target=cpu` の場合は CPU resolved image、`target=gpu` の場合は GPU resolved image
- **extra_hosts**: 
  ```yaml
...
- `host.has_gpu=true` の場合のみ ray-gpu が起動

**期待される動作**:
```
1. health → 起動 (PORT 8080)
2. ray-cpu → health 確認 → 既存 HEAD なし → HEAD として起動
3. ray-gpu（存在時）→ health 確認 → ray-cpu を発見 → WORKER として起動
```
